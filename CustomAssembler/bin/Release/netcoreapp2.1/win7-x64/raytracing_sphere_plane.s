...
;
        ...
        mov c_x,d
        call _calculateShade
        mov ou,a
        dshr a ; truncate to 6 bits
        qshl a
        shl a
        mov a,ou
    ...
    ; replace beginning of program with a couple halts in case of reset
    mov 1,a
    mov a,*0
    mov a,*1
    mov a,*2
...
; plane at 0,-1.5,0 with normal (0,1,0)
_intersectPlane:
    mov c,*rr0 ; &rayOrigin
    mov d,*rr1 ; &rayDir
...
_calculateShade:
    ...
    ; return clamp(a, 0.125, 1.0);
    cmp a,32
    jns calculateShade_ns
        mov 32,ou
        ret
    calculateShade_ns:
